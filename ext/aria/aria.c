...
static int EncKeySetup(const Byte *w0, Byte *e, int keyBits) {
...
static int DecKeySetup(const Byte *w0, Byte *d, int keyBits) {
	int  i, j, R;
	Byte t[16];
...
static void Crypt(const Byte *p, int R, const Byte *e, Byte *c)
{
	int i, j;
	Byte t[16];
...
#define CLASS_NAME "ARIAEngine"

struct ARIA_CTX {
  int keyBits;
  int numberOfRounds;
  Byte masterKey[32];
  Byte encRoundKeys[272];
  Byte decRoundKeys[272];
};


static void aria_free(struct ARIA_CTX * p) {
  ruby_xfree(p);
}

static VALUE aria_alloc(VALUE klass) {
  return Data_Wrap_Struct(klass, NULL, aria_free, ruby_xmalloc(sizeof(struct ARIA_CTX)));
}

static VALUE aria_init(VALUE self);

static VALUE aria_reset(VALUE self);
static VALUE aria_set_key(VALUE self, VALUE key_size);
static VALUE aria_encrypt(VALUE self, VALUE plain);
static VALUE aria_decrypt(VALUE self, VALUE cipher);
static VALUE aria_inspect(VALUE self);

void Init_aria() {
  VALUE aria = rb_define_class(CLASS_NAME, rb_cObject);
  rb_define_alloc_func(aria, aria_alloc);
  rb_define_private_method(aria, "initialize", RUBY_METHOD_FUNC(aria_init), 0);
  rb_define_method(aria, "reset", RUBY_METHOD_FUNC(aria_reset), 0);
  rb_define_method(aria, "key=", RUBY_METHOD_FUNC(aria_set_key), 1);
  rb_define_method(aria, "encrypt", RUBY_METHOD_FUNC(aria_encrypt), 1);
  rb_define_method(aria, "decrypt", RUBY_METHOD_FUNC(aria_decrypt), 1);
  rb_define_method(aria, "inspect", RUBY_METHOD_FUNC(aria_inspect), 0);
}
...
static VALUE aria_reset(VALUE self) {
  struct ARIA_CTX * p;
  Data_Get_Struct(self, struct ARIA_CTX, p);
...
static VALUE aria_set_key(VALUE self, VALUE key) {
  struct ARIA_CTX * p;
  char * c_key = RSTRING_PTR(key);
  int c_key_len = RSTRING_LEN(key);
  Data_Get_Struct(self, struct ARIA_CTX, p);

  if(c_key_len < 16) {
    rb_raise(rb_eArgError, "Key must be longer than 16");
    return Qnil;
  } else {
    aria_reset(self);
    if(c_key_len < 24) p->keyBits = 128;
    else if(c_key_len < 32) p->keyBits = 192;
    else p->keyBits = 256;
  }

  memset(p->masterKey, 0, 32);
  memcpy(p->masterKey, c_key, c_key_len < 32 ? c_key_len : 32);

  p->numberOfRounds = EncKeySetup(p->masterKey, p->encRoundKeys, p->keyBits);
  DecKeySetup(p->masterKey, p->decRoundKeys, p->keyBits);
...
static VALUE aria_encrypt(VALUE self, VALUE plain) {
  struct ARIA_CTX * p;
  Byte c_plain[16] = {0, }, c_cipher[16] = {0, };
  int c_plain_len = RSTRING_LEN(plain);
  Data_Get_Struct(self, struct ARIA_CTX, p);

  memcpy(c_plain, RSTRING_PTR(plain), c_plain_len < 16 ? c_plain_len : 16);
  Crypt(c_plain, p->numberOfRounds, p->encRoundKeys, c_cipher);

  return rb_str_new((char *)c_cipher, 16);
}

static VALUE aria_decrypt(VALUE self, VALUE cipher) {
  struct ARIA_CTX * p;
  Byte c_plain[16] = {0, }, c_cipher[16] = {0, };
  int c_cipher_len = RSTRING_LEN(cipher);
  Data_Get_Struct(self, struct ARIA_CTX, p);

  memcpy(c_cipher, RSTRING_PTR(cipher), c_cipher_len < 16 ? c_cipher_len : 16);
  Crypt(c_cipher, p->numberOfRounds, p->decRoundKeys, c_plain);

  return rb_str_new((char *)c_plain, 16);
}
...
static VALUE aria_inspect(VALUE self) {
  ...
  for(i = 0; i < 32; i ++) {
    masterKey[i * 2] = digits[(p->masterKey[i] & 0xf0) >> 4];
    masterKey[i * 2 + 1] = digits[p->masterKey[i] & 0xf];
  }
  switch(p->keyBits) {
    case 128:
      masterKey[32] = 0;
      return rb_sprintf("#<" CLASS_NAME " 128bits R=%d masterKey=%s>", p->numberOfRounds, masterKey);
    case 192:
      masterKey[48] = 0;
      return rb_sprintf("#<" CLASS_NAME " 192bits R=%d masterKey=%s>", p->numberOfRounds, masterKey);
    case 256:
      masterKey[64] = 0;
      return rb_sprintf("#<" CLASS_NAME " 256bits R=%d masterKey=%s>", p->numberOfRounds, masterKey);
    default:
      return rb_str_new_cstr("#<" CLASS_NAME " nil>");
  }
}
